import type { RaceCarState, RaceControlStatus, StrategyIntent, TyreCompound } from "@/domain/race";
...
export function buildAiStrategyDecision(context: AiStrategyContext, car: RaceCarState): AiStrategyDecision {
  ...
  const wetTarget: TyreCompound | null = context.trackWetness >= 0.58
    ? "WET"
  ...
  const ranked = compounds
    .map((compound) => {
      ...
      const switchingBonus = wetTarget === compound ? 18 : 0;
  ...
  const confidence = Math.max(0.35, Math.min(0.96, 0.52 + margin * 0.018 + (weatherMismatch ? 0.16 : 0) - (doubleStackRisk ? 0.12 : 0)));
