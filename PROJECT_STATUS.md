...
Project Pitwall은 **Silverstone 단일 레이스 위크엔드를 처음부터 끝까지 플레이할 수 있는 데스크톱 웹 프로토타입** 단계입니다.
...
| 다중 서킷·시즌·커리어 | 미구현 | 다음 제품 확장 범위 |
| 온라인·서버 저장·오디오·3D | 미구현 | 프로토타입 이후 범위 |
...
- 여러 서킷과 시즌 캘린더
- 드라이버·컨스트럭터 챔피언십 포인트와 시즌 순위
- 팀 재정, 연구개발, 시설, 계약, 직원과 장기 커리어
- 시즌 전체 타이어 할당과 파워유닛·기어박스 부품 수명
- 열 위험을 넘어선 상세 기계 고장·수리·부품 교체
- 장기 세이브·불러오기와 경기 결과 내보내기
- 온라인 멀티플레이와 서버 저장
- 3D 레이스 화면, 오디오, 중계 연출
...
- UI 문자열 및 콘텐츠 팩 분리
- 핵심 상태에 대한 저장·복구 경계 테스트

## 7. 권장 다음 개발 순서

1. **다중 서킷 데이터 계층**: 트랙 지오메트리·텔레메트리·공력/에너지 규정 프로파일을 설정 데이터로 분리합니다.
2. **챔피언십 루프**: 주말 결과를 포인트·시즌 순위·다음 라운드로 연결합니다.
3. **팀 운영 메타게임**: 예산·시설·직원·개발을 레이스 정확도와 성능에 연결합니다.
4. **신뢰성 계층**: 부품 수명, 고장 확률, 수리 및 교체 비용을 추가합니다.
5. **세이브·CI·성능**: 장기 세이브 스키마, 브라우저 자동화, 장시간 안정성 검증을 고정합니다.
